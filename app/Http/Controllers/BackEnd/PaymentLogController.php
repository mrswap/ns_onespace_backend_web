...
class PaymentLogController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->status === "1") {
            ...
            if ($count_membership > 1) {
                ...
                $mailType = 'packagePurchaseMembershipAccepted';
            } else {
            ...
            if ($count_membership > 1) {
                ...
                $mailType = 'packagePurchaseMembershipRejected';
            } else {
    ...
    public function refundupdate(Request $request)
    {
        ...
        $package = Package::query()->where('id', $membership->package_id)->first();
        $count_membership = Membership::query()->where('vendor_id', $membership->vendor_id)->count();
        $membership->update(['refund_status' => $request->refund_status]);
        session()->flash('success', "Membership Refund changed successfully!");
        return back();
    }
