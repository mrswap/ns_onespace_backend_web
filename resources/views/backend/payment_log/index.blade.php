                    ...
                    <tbody>
                      @foreach ($memberships as $key => $membership)
                        <tr>
                          ...
                          <td>
                            @if (json_decode($membership->transaction_details) !== 'offline')
                              @if ($membership->refund_status == 1)
                                <h3 class="d-inline-block badge badge-success">{{ __('Success') }}</h3>
                              @elseif ($membership->refund_status == 0)
                                <h3 class="d-inline-block badge badge-warning">{{ __('Pending') }}</h3>
                              @elseif ($membership->refund_status == 2)
                                <h3 class="d-inline-block badge badge-danger">{{ __('Rejected') }}</h3>
                              @endif
                            @else
                              <form id="refundstatusForm{{ $membership->id }}" class="d-inline-block"
                                action="{{ route('admin.refund-log.update') }}" method="post">
                                @csrf
                                <input type="hidden" name="id" value="{{ $membership->id }}">
                                <select
                                  class="form-control form-control-sm
                                                    @if ($membership->refund_status == 1) bg-success
                                                    @elseif ($membership->refund_status == 0)
                                                    bg-warning
                                                    @endif
                                                    "
                                  name="refund_status"
                                  onchange="document.getElementById('refundstatusForm{{ $membership->id }}').submit();">
                                  <option value=0 {{ $membership->refund_status == 0 ? 'selected' : '' }}>{{ __('Pending') }}
                                  </option>
                                  <option value=1 {{ $membership->refund_status == 1 ? 'selected' : '' }}>{{ __('Success') }}
                                  </option>
